...
def _get_participants_per_range(duels: dict[Range, list[Duel]]) -> dict[str, list[Participant]]:
    result = {}
...
def _deliver_participants(participants: typing.Iterable[Participant], excel_writer: pd.ExcelWriter):
    df = (
        pd.DataFrame(
            [
                {
...
def deliver_sorted_pairs(range_queues: dict[Range, list[Duel]], excel_writer: pd.ExcelWriter):
